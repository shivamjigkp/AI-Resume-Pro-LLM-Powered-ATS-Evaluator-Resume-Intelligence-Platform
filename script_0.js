...
async function fetchAndRenderGate1Leads(){
  ...
  const sel = document.getElementById('gate1ActiveSheet');
  const sheetId = extractCleanSheetId(sel?.value || '1lYkZAjqQQQGKTkxkLGUpNmcs4Wu68tPXo6JRa0PDimI');

  box.innerHTML = '<div style="padding:10px;color:#6366f1;text-align:center;font-weight:600;">🔄 Fetching real Google Sheet rows...</div>';

  try {
    const res = await fetch('/api/gate1/sheet-rows?sheet_id=' + encodeURIComponent(sheetId));
    const data = await res.json();
    ...
    if(data.status === 'success' && Array.isArray(data.rows) && data.rows.length > 0){
          ...
          <tbody>
            ${rows.map(l => `
              <tr style="border-bottom:1px solid #e2e8f0;background:#fff;">
                ...
                <td style="padding:5px 8px;font-weight:600;">${l.name || 'Hiring Team'}</td>
                ...
                <td style="padding:5px 8px;"><span style="font-size:9.5px;padding:2px 5px;border-radius:4px;background:#e0f2fe;color:#0369a1;font-weight:700;">${l.status}</span></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      box.innerHTML = `<div style="padding:10px;color:#dc2626;text-align:center;">No active rows found in sheet "${sheetId.substring(0,12)}..."</div>`;
    }
  } catch(e) {
    box.innerHTML = '<div style="padding:10px;color:#dc2626;text-align:center;">Error connecting to backend Google Sheet service.</div>';
  }
}
