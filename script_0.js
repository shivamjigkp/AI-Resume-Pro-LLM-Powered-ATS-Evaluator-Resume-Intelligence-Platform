...
function addNewGate1TemplatePrompt(){
  const title = prompt("Enter a display title for the new template (e.g. Frontend React Pitch):");
  if(!title || !title.trim()) return;

  const templates = getStoredTemplates();
  const nextId = String(templates.length + 1);
  const cleanKey = title.trim().toLowerCase().replace(/[^a-z0-9_]/g, '_');
  
  const newEntry = { id: nextId, name: cleanKey, label: `⭐ ${nextId} — ${title.trim()}` };
  templates.push(newEntry);
  saveStoredTemplates(templates);

  const defaultTplText = `Subject: Application for {{role}} at {{company}}\n\nDear {{recruiter_name}},\n\nI hope you are doing well.\n\nI am writing to express my strong interest in internship and full-time opportunities at {{company}} for the {{role}} position...\n\nBest regards,\nShivam Gupta\n+91-8081513780 | quantxcoder@gmail.com`;
  
  ALL_MASTER_TEMPLATES[nextId] = defaultTplText;
  ALL_MASTER_TEMPLATES[cleanKey] = defaultTplText;

  localStorage.setItem('rsai_active_template', nextId);
  populateTemplateDropdown();
  ...
  syncGate1LiveTemplateEditor();
  toast(`➕ Created new template "${title.trim()}"! Edit and click 💾 Save.`, 'success', 4000);
}

function deleteActiveGate1Template(){
  const sel = document.getElementById('gate1DefaultTemplate');
  const activeVal = sel?.value;
  if(!activeVal){ toast('No active template selected', 'warning'); return; }
  ...
  const targetIdx = templates.findIndex(t => t.id === activeVal || t.name === activeVal);
  if(targetIdx === -1){
    toast('Template not found', 'warning');
    return;
  }

  const targetName = templates[targetIdx].label || templates[targetIdx].name;
  if(!confirm(`Are you sure you want to delete "${targetName}"?`)) return;

  templates.splice(targetIdx, 1);
  saveStoredTemplates(templates);
  
  const nextActive = templates[0]?.id || '1';
  localStorage.setItem('rsai_active_template', nextActive);
  populateTemplateDropdown();
  
  if(sel) sel.value = nextActive;
  syncGate1LiveTemplateEditor();
  toast(`🗑️ Deleted template "${targetName}"`, 'info', 3000);
}
...
const MASTER_TEMPLATES_COLLECTION = [
  ...
  { id: '12_mastermind_extended', name: '⭐ 12 — Mastermind Extended (5-Min Call CTA)' }
];
...
function populateTemplateDropdown(){
  ...
  // Combine builtin master templates with custom user-created templates
  const customTpls = getStoredTemplates().filter(ct => !MASTER_TEMPLATES_COLLECTION.some(m => m.id === ct.id || m.id === ct.name));
  const allList = [...MASTER_TEMPLATES_COLLECTION, ...customTpls];

  sel.innerHTML = allList.map(t => {
    const tId = t.id || t.name;
    const tName = t.name || t.label || tId;
    const isSel = (tId === activeTpl || tId === activeTpl.replace('.txt','')) ? 'selected' : '';
    return '<option value="' + tId + '" ' + isSel + '>' + tName + '</option>';
  }).join('');
}
